...
class CoreTests(unittest.TestCase):
    ...
    def test_run_scan_and_alert_integration_once(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            history_path = str(Path(tmpdir) / "picks.csv")
            with patch.dict(
                "os.environ",
                {
                    "TOP_COUNT": "5",
                    ...
                    "BACKTEST_HISTORY_FILE": history_path,
                },
                clear=False,
            ):
                plans = run_scan_and_alert(send_email=False)
